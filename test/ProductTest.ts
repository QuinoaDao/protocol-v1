...
//     const {product, wmaticStrategy, wethStrategy, ghstStrategy, quickStrategy, usdcStrategy, usdPriceModule, swapModule, nonDacStrategy, diffAssetStrategy, dupStrategy} = await deployContracts();
...
//     // usdPriceModue, swapModule 확인
//     expect(await product.currentUsdPriceModule()).equal(usdPriceModule.address);
//     expect(await product.currentSwapModule()).equal(swapModule.address);
...
//     console.log("current withdrawal queue state: ", product.withdrawalQueue);
//   })
...
//   it('updateSwapModule test', async() => {
//     const {
//       dac, nonDac,
//       product, wmaticStrategy,
//       wethStrategy, ghstStrategy,
...
//     expect(product.updateSwapModule(ethers.constants.AddressZero)).to.be.revertedWith("Invalid swap module");
//     expect(product.updateSwapModule(swapModule.address)).to.be.revertedWith('Duplicated Vaule input');
//     expect(product.connect(nonDac).updateSwapModule(ethers.constants.AddressZero)).to.be.revertedWith('Only dac can access');
//     await product.updateSwapModule(nonDac.address);
//     expect(await product.currentSwapModule()).equal(nonDac.address);
//   })
