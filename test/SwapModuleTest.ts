...
// @잰>밍 approve 를 날리는 주체는 dac, swap 을 call 하는 주체는 product contract 이므로 해당 테스트는 fail 이 납니다.
// @잰>밍 swap module 함수만 분리하여 별도로 테스트하는 방법은 없어 보입니다. product 함수들을 테스트해야 할 것 같습니다.
describe("swapModule test", async () => {
  it("wmatic - usdc convert: swapExactOutput", async () => {
    ...
    console.log(
      "wmatic price: ",
      (await usdPriceModule.getAssetUsdPrice(wmaticAddress)).toString()
    );
    console.log(
      "usdc price: ",
      (await usdPriceModule.getAssetUsdPrice(usdcAddress)).toString()
    );

    console.log(
      "Before wMatic balance",
      await wMaticContract.balanceOf(dac.address)
    );
    console.log(
      "Before usdc balance",
      await usdcContract.balanceOf(dac.address)
    );

    let amountInEstimated = await product.estimateSwapInputAmount(
    ...
    console.log(
      "balance of product : ",
      await wMaticContract.balanceOf(product.address)
    );
    console.log(
      "allowance: ",
      await wMaticContract.allowance(dac.address, wmatic_usdc)
    );

    await product.swapExactOutput(
      ethers.utils.parseUnits("100", 6),
      wmaticAddress,
      usdcAddress,
      dac.address
  ...
  it("wmatic - quick convert", async () => {
    ...
    console.log(
      "quick price: ",
      (await usdPriceModule.getAssetUsdPrice(quickAddress)).toString()
    );
    ...
    console.log(
      "Before quick balance",
      await quickContract.balanceOf(dac.address)
    );
    ...
    console.log(
      "allowance: ",
      await wMaticContract.allowance(dac.address, wmatic_quick)
    );
