...
describe("Weth strategy test", async () => {
    async function deployAndSetting(signers: SignerWithAddress[]) {
        ...
        const {
            ...
            usdPriceModule,
            whitelistRegistry
        } = await utils.deployWithCustomInfo(signers[0], productInfo);
    ...
    it.only("Weth strategy.deposit test",async () => {
        const {
            signers,
            product,
            wmaticStrategy,
        ...
        console.log("user deposit & rebalance end");

        // call strategy.desposit function
        await wethStrategy.deposit();

        // Check the deposit logic using moo balance
        const mooWethContract = new ethers.Contract("0x1d81c50d5aB5f095894c41B41BA49B9873033399", utils.wEthAbi, signers[0]); // erc20(weth)
        const bptWethContract = new ethers.Contract("0x65Fe9314bE50890Fb01457be076fAFD05Ff32B9A", utils.wEthAbi, signers[0]); // erc20(weth)

        expect(await wEthContract.balanceOf(wethStrategy.address)).equal(0, "weth token error")
        ...
        console.log("strategy deposit end");
        ...
        console.log("user withdraw end");

    });

    it("Weth strategy.withdrawAll test",async () => {
        const {
            signers,
            product,
            wmaticStrategy,
