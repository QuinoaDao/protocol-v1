...
describe('weth in weth out test',async () => {
    it('weth in - weth out',async () => {
        ...
        await swapTokens(dac, nonDac, wMaticContract, wEthContract, usdcContract);
        ...
        await product.connect(nonDac).deposit(wethAddress, parseUnits("25", 15), nonDac.address);
        await product.connect(nonDac).deposit(wethAddress, parseUnits("25", 15), nonDac.address);
        await product.connect(nonDac).deposit(wethAddress, parseUnits("25", 15), nonDac.address);
        await product.connect(nonDac).deposit(wethAddress, parseUnits("25", 15), nonDac.address);
        ...
        console.log("product weth balance: ", await product.assetFloatBalance(wethAddress));

        // user의 share token 잔액 확인
        console.log("nonDac's deposit value: ", await usdPriceModule.getAssetUsdValue(wethAddress, parseUnits("1", 17)));
        ...
        console.log("user's total withdrawal value: ", await usdPriceModule.getAssetUsdValue(wethAddress, (await wEthContract.balanceOf(nonDac.address)).sub(nonDacWethBalance)));
    })
    ...
    it('weth in - usdc out',async () => {
        ...
        console.log("product's total usdc balance: ", await product.assetBalance(usdcAddress));
        console.log("product's total portfolio value: ", await product.portfolioValue());
        console.log("user's total withdrawal value: ", await usdPriceModule.getAssetUsdValue(usdcAddress, (await usdcContract.balanceOf(nonDac.address)).sub(nonDacUsdcBalance)));
    })
