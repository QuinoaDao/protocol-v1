...
contract testProduct is ERC20, IProduct, SwapModule {
    ...
    function withdraw(address assetAddress, uint256 shareAmount, address receiver, address owner) external override returns (uint256) {
        ...
        console.log(assetAddress, shareAmount, balanceOf(_msgSender()), withdrawalAmount);
        ...
        if (_assetBalanceOf(assetAddress, address(this)) < withdrawalAmount) {
            console.log("float is insufficient (we need swap in float): ", _assetBalanceOf(assetAddress, address(this)));
            ...
            for (uint i=0; i<assets.length; i++){ // Withdraw tokens from the another float
                ...
                console.log(floatAssetAddress, needAmount, _assetBalanceOf(floatAssetAddress, address(this)));
                ...
                if(needAmount > _assetBalanceOf(floatAssetAddress, address(this))) {
                    ...
                    if(_estimateSwapOutputAmount(needAmount, floatAssetAddress, assetAddress) == 0) { // There is no float
                        console.log("there is no float");
                        continue;
                    }
                    
                    console.log("swap exact input in float: ", needAmount, _assetBalanceOf(floatAssetAddress, address(this)));
                    ...
                    console.log("swap result: ", _assetBalanceOf(assetAddress, address(this)));
                }
                else {
                    console.log("swap exact output in float: ", needAmount, _assetBalanceOf(floatAssetAddress, address(this)));
                    IERC20(floatAssetAddress).approve(address(router), needAmount);
            ...
            for (uint i=0; i<withdrawalQueue.length; i++){ // Withdraw tokens from the strategy
                ...
                console.log("float is insufficient (we need swap in strategy): ", floatAmount);
                ...
                if(assetAddress == strategyAssetAddress) {
                    ...
                    if(needAmount == 0) {
                        console.log("there is no token in strategy");
                        continue;
                    }
                    
                    console.log("redeem from strategy", strategyAssetAddress, needAmount);
                    ...
                    if(needAmount > IStrategy(withdrawalQueue[i]).totalAssets()) {
                        ...
                        console.log(_estimateSwapOutputAmount(needAmount, strategyAssetAddress, assetAddress));

                        console.log("swap exact input in strategy: ", strategyAssetAddress, needAmount, IStrategy(withdrawalQueue[i]).totalAssets());
                        _redeemFromStrategy(withdrawalQueue[i], needAmount);
                        ...
                        console.log("swap exact output in strategy: ", strategyAssetAddress, needAmount, IStrategy(withdrawalQueue[i]).totalAssets());
                        _redeemFromStrategy(withdrawalQueue[i], needAmount);
                        IERC20(strategyAssetAddress).approve(address(router), needAmount);
